...
pro BOREALIS_timefreqcorrection,tmin,N_time,steps,fmin,fmax,beam,S,file_root,date,save_file_name,$
                     ...
                     LOFAR=LOFAR,NENUFAR=NENUFAR,DYNSPECMS=DYNSPECMS,filename=filename,COMPLEX_NAME=COMPLEX_NAME,$
                     TARGET=TARGET,FULL=FULL,TO=TO,Period=Period,PCA=PCA
  ...
  ;**********************************************
     If keyword_set(TimeFreq_correction) then begin
       ...
       If not(keyword_set(mask_bit)) and not(keyword_set(bit)) then begin   
         ...
         If keyword_set(use_combinemask) then begin 
            If keyword_set(NENUFAR) then save_file_name3 = date+'_'+planet+'_pol'+strtrim(string(S),1) 
             If keyword_set(LOFAR) then save_file_name3 = strmid(save_file_name,0,strlen(save_file_name)-6); e.g. L429868_pol0_41sec_RFI_patrol5.5_pex_lesig3.5_sum
    ...
    Endif
      ...
      ;------------------------------------------------------------------------------------------------
      ;------------------------------------------------------------------------------------------------
      ;                               Method 1 (LOFAR): Line by line curve fitting
      ;-----------------------------------------------------------------------------------------------
      ;------------------------------------------------------------------------------------------------
      If not(keyword_set(Process_METHOD2)) or keyword_set(method2_again) then begin
          ...
          If keyword_set(NENUFAR) then begin
            ...
            READ_NU_FITS, file_input, command,param,variab, nt,dt,nf,df,ns,jd0,h0,fref,$
              x,t_full,xf,beam,ndata,corrt,corrf,/quiet,nstokes=3
...
;             	  endif 
     ...
     ;------------------------------------------------------------------------------------------------
     ;------------------------------------------------------------------------------------------
     ;                Method 2 (LOFAR): Fitting Frequency Response (10% Quantile) over time
     ;------------------------------------------------------------------------------------------
     ;------------------------------------------------------------------------------------------------
        If keyword_set(Process_METHOD2) then begin
          print,'*************************************************'
...
;%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
;%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pro BOREALIS_ApplyCorrections,tmin,N_time,steps,fmin,fmax,beam,S,file_root,date,save_file_name,$
                     disp_values=disp_values,$
                     PS=PS,VERBOSE=VERBOSE,$
                     rebin_freq=rebin_freq, rebin_time=rebin_time,rebin_data=rebin_data,$
                     TimeFreq_correction=TimeFreq_correction,Apply_Corrections=Apply_Corrections,skip_rebinloop=skip_rebinloop,$
      ...
      If S eq 4 then S_string = strtrim(string(uint(3)),1)   ; V^2
      If S eq 5 then S_string = strtrim(string(uint(3)),1)   ; V'
      If S eq 6 then S_string = strtrim(string(uint(1)),1)   ; L = sqrt(Q^2 + U^2)
     ...
     If keyword_set(Apply_Corrections) then begin
       ...
       If keyword_set(NENUFAR) then begin
        ...
        file_input = findfile(temp)
        READ_NU_FITS, file_input, command,param,variab, nt_full,dt,nf,df,ns,jd0,h0,fref,$
          temporary(data3),xt_full,xf,beam_temp,ndata,corrt,corrf,/nodata,/quiet
        sampling_time = dt
        channel_width = df   ; unis of kHz 
        channel_width = channel_width*1000.  ; units of Hz
       Endif
       If keyword_set(DYNSPECMS) then begin
         If S eq 0 then POL = 1 ;Stokes-I
         If S eq 1 then POL = 2 ;Stokes-Q
         ...
         If S eq 4 then POL = 4 ;Stokes-V^2
         If S eq 5 then POL = 4 ;Stokes-V prime
         read_dynspec_fits,filename+'_beam'+B_string,0,1,fmin,fmax,POL,data,time,xf,nt,nf,dt=dt,df=df,tall=tf
         sampling_time = dt
         channel_width = df
       Endif
       ;----------------------------------------------
       ...
       If keyword_set(LOFAR) then begin
         ...
         If S eq 4 or S eq 5 then begin 
          ...
          If not(keyword_set(COMPLEX_NAME)) then begin
            If keyword_set(NENUFAR) then begin
              save_file_name3 = date+'_'+Target+'_pol'+strtrim(string(0),1)+'_beam'+b_String
            Endif ELSE BEGIN
              save_file_name3 = date+'_pol'+strtrim(string(0),1)+'_beam'+b_String
            Endelse
          Endif
       ...
       If not(keyword_set(mask_bit)) then begin
         If not(keyword_set(use_combinemask)) then begin
          print,'Restore Mask: ', 'mask_full_'+save_file_name+'.sav'
           restore,filename='mask_full_'+save_file_name+'.sav'
             ;xt,xf,p2_full
             ;xt,xf,p2_full,jd0,mask_nenufar fro NenuFAR
         endif; not combined mask 
      ...
      If keyword_set(PS) and keyword_set(rebin_data) then begin
       ...
       If keyword_set(NENUFAR) then begin
         ...
         If S eq 3 or S eq 4 then begin
          If not(keyword_set(use_combinemask)) then begin
           save_file_name_I = date+'_'+TARGET+'_pol0_'+'beam'+strtrim(string(cgnumber_formatter(beam[0],decimals=0)),1)
           ;restore,filename='mask_full_'+save_file_name_I+'.sav'
           ;p2_full_I = p2_full
           ;xt,xf,p2_full,jd0,mask_nenufar
           ;p2_full_I = p2_full
           
           ;mask_nenufar_I = p2_full
           ;data_full_I = x[*,*,0]
           ;mask_nenufar = ndata[*,*]
           ;p2_combine = mask_nenufar*mask_nenufar_I
           endif 
           If keyword_set(use_combinemask) then begin
            print,'Combine Mask'
            filename = 'mask_full_'+date+'_'+TARGET+'_pol0.sav'
       ...
       ;------------------
       ;PCA analysis (on full data)
       ;------------------
       If keyword_set(NENUFAR) and keyword_set(PCA) then begin
         print, '------------------------'
         print, '        PCA             '
         print, '------------------------'  
         borealis_applypcaCorrectionsv2,tmin,N_time,steps,fmin,fmax,beam,S,file_root,date,save_file_name,$
           /PS,/VERBOSE,$
           /TimeFreq_correction,/Apply_Corrections,planet=planet,$
           /NENUFAR,filename=filename,N_SYSREM=N_SYSREM,data=data
         ;data is output

         ;borealis_applypcaCorrectionsv2
         p2 = P2_FULL
         label = 'Data: After PCA'
         STANDARD_PLOTS, data/(p2*1. + (p2*1. eq 0)),p2*1.,t_full,xf,label,xunit = 'secs',panel_ps=1
       ...
       for kl=0,steps-1 do begin  ;begin time loop-
        ...
        If keyword_set(NENUFAR) then begin
         ...
         If keyword_set(DYNSPECMS) then begin
           ...
           If S eq 4 then POL = 4 ;Stokes-V^2
           If S eq 5 then POL = 4 ;Stokes-V prime
           read_dynspec_fits,filename+'_beam'+B_string,tmin,tmax,fmin,fmax,POL,data,time,xf,nt,nf         
         Endif
       ...
       ;Apply Time Correction Correction to LOFAR data
       If keyword_set(LOFAR) then begin
         ...
         If not(keyword_set(Smooth_Surface)) then begin
           ...
           If not(keyword_set(gaussian_surface)) then begin
            ...
            If keyword_set(LOFAR) or keyword_set(NENUFAR) then begin   ;edit
              If S ne 5 and S ne 4 then begin 
                print,'S: (divide)'
            ...
            endif ;end LOFAR and Nenufar ;edit      
          endif  ;end not(keyword_set(gaussian_surface))   
         Endif ; end not(keyword_set(Smooth_Surface))
         ...
         endif ;end Smooth_Surface surface 
       endif ;end apply surface to LOfAR
        
       If keyword_set(DynspecMS) or keyword_set(NENUFAR) then data = data*p2
       ...
       ;----------------------
        ...
        If keyword_set(DynspecMS) then begin
          data_rebin_p[kl]  = ptr_new(data)
          p2_rebin_p[kl]    = ptr_new(p2)
          xt_rebin_p[kl]    = ptr_new(time)
        Endif
        ...
        If keyword_set(rebin_data) then begin
          ...
          ;------------------------
          If keyword_set(NENUFAR) then begin
            reduce_array,data*p2*mask_nenufar,[Num_rebin_t,Num_rebin_f],data_rebin2,/dbl
            reduce_array,p2*1.*mask_nenufar,[Num_rebin_t,Num_rebin_f],p2_rebin2,/dbl   ; multiply masks for NenuFAR
          Endif
          If keyword_set(LOFAR) then begin
     ...
     for j=0,steps-1 do begin ;n dates
      ...
      ;phase setup
      If keyword_set(LOFAR) then begin
        JD_rebin = MJD_REBIN + 2400000.50d
        phase_rebin = (((MJD_rebin - TO) mod Period)/(Period*1.d))
      endif  
      If keyword_set(NENUFAR) then begin
        nt_new = n_elements(xt_rebin)
...
;--------------------------------------------------
;--------------------------------------------------
pro BOREALIS_processing,tmin,N_time,steps,fmin,fmax,beam,S,file_root,date,save_file_name,$
  disp_values=disp_values,$
  PS=PS,VERBOSE=VERBOSE,$
  rebin_freq=rebin_freq, rebin_time=rebin_time,rebin_data=rebin_data,$
  TimeFreq_correction=TimeFreq_correction,Apply_Corrections=Apply_Corrections,skip_rebinloop=skip_rebinloop,$
  ...
  ;************************************************
  If keyword_set(LOFAR) then begin
    BOREALIS_timefreqcorrection,tmin,N_time,steps,fmin,fmax,beam,S,file_root,date,save_file_name,$
    disp_values=disp_values,$
    PS=PS,VERBOSE=VERBOSE,$
    rebin_freq=rebin_freq, rebin_time=rebin_time,rebin_data=rebin_data,$
    TimeFreq_correction=TimeFreq_correction,Apply_Corrections=Apply_Corrections,skip_rebinloop=skip_rebinloop,$
    ...
    TimeFreqCorrect_array=TimeFreqCorrect_array,$
    Run_JUPITER=Run_JUPITER,REMOVE_BADF=REMOVE_BADF,N_smooth=N_smooth,Process_METHOD2=Process_METHOD2,$
    LOFAR=LOFAR,NENUFAR=NENUFAR,$
    DYNSPECMS=DYNSPECMS,filename=filename,COMPLEX_NAME=COMPLEX_NAME,$
    TARGET=TARGET,FULL=FULL,TO=TO,Period=Period,PCA=PCA
  endif 
  ...
  ;************************************************
  BOREALIS_ApplyCorrections,tmin_or,N_time,steps,fmin,fmax,beam,S,file_root,date,save_file_name,$
    disp_values=disp_values,$
    PS=PS,VERBOSE=VERBOSE,$
    rebin_freq=rebin_freq, rebin_time=rebin_time,rebin_data=rebin_data,$
    TimeFreq_correction=TimeFreq_correction,Apply_Corrections=Apply_Corrections,skip_rebinloop=skip_rebinloop,$
