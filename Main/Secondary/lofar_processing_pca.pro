...
;%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
;%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pro lofar_ApplyPCACorrections,tmin,N_time,steps,fmin,fmax,beam,S,file_root,date,save_file_name,$
  ...
  If keyword_set(Apply_Corrections) then begin
    ...
    ;----------------------------------------------
    ;----------------------------------------------
    MJD_t = dindgen(nof_samples,start=START_MJD,INCREMENT=MJD_sampling_time)
    tmin= tmin_or
    ...
    for kl=0,steps-1 do begin  ;begin time loop-
      ...
      If S eq 4 or S eq 5 then begin ;V'
        ...
        If kl eq 0 then STANDARD_PLOTS, data,p2*1.,time,xf,'V/I',xunit = 'secs',panel_ps=1
...
;        If not(keyword_set(gaussian_surface)) then begin
     ;     data = data*p2   ;apply mask 
          ...
          If kl eq 0 then STANDARD_PLOTS, data_S,p2,time,xf,'S: Pre-Sysrem',/ONLY_PLOT,/NO_ZOOM
          for i=0,0 do begin
            print,'I Systemaic: ',i
            sysrem,data_S,error,sys_err=sys_err
            data_S = data_S - sys_err
            If kl eq 0 then STANDARD_PLOTS, data_S,p2,time,xf,'S: Sysrem Number; '+strtrim(string(i),1),/NO_ZOOM
          endfor
          ...
          ;If S eq 5 or S eq 4 then begin
            ...
            If S eq 4 then begin 
              ...
              data = data*data_S_I*p2              ;final Vhat
...
pro lofar_processing_pca,tmin,N_time,steps,fmin,fmax,beam,S,file_root,date,save_file_name,$
  disp_values=disp_values,$
  PS=PS,VERBOSE=VERBOSE,$
  rebin_freq=rebin_freq, rebin_time=rebin_time,rebin_data=rebin_data,$
  TimeFreq_correction=TimeFreq_correction,Apply_Corrections=Apply_Corrections,skip_rebinloop=skip_rebinloop,$
  ...
  lofar_ApplyPCACorrections,tmin_or,N_time,steps,fmin,fmax,beam,S,file_root,date,save_file_name,$
    disp_values=disp_values,$
    PS=PS,VERBOSE=VERBOSE,$
    rebin_freq=rebin_freq, rebin_time=rebin_time,rebin_data=rebin_data,$
    TimeFreq_correction=TimeFreq_correction,Apply_Corrections=Apply_Corrections,skip_rebinloop=skip_rebinloop,$
