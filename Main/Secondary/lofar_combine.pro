...
;
pro lofar_combine,file_root,save_file_name,dates,dates_filenames,beam,LIST=LIST;,save_file_name2=save_file_name2


out_ps = 'combine_'+save_file_name+'_beam'+strtrim(string(cgnumber_formatter(beam,decimals=0)),1)
set_plot,'PS'
device,filename=out_ps+'.ps',/landscape
  ...
  for i=0,n-1 do begin 
    restore,filename=file_root+'/'+dates[i]+'/rebindata_'+$
   ...
   ;setup pointers 
    ...
    ;Concatenate arrays 
    If i eq 0 then xt_rebin_all =  *xt_rebin_p[i]
    If i gt 0 then xt_rebin_all =  [ xt_rebin_all,*xt_rebin_p[i] ]  
    If i eq 0 then mjd_rebin_all =  *mjd_rebin_p[i]
    If i gt 0 then mjd_rebin_all =  [ mjd_rebin_all,*mjd_rebin_p[i] ]
