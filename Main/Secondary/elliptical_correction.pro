...
;
;Version 2.1: Dec 19, 2018 (JMaG)
;		* includes RENORM=RENORM flag
;
pro elliptical_correction,x,y,xout=xnew,yout=ynew, b1out=b1, b2out=b2, VERBOSE=VERBOSE, RENORM=RENORM

; --------------------------------------------
; default options

if not(keyword_set(RENORM)) then RENORM=1.
...
if keyword_set(VERBOSE) then print,'orientation (before correction)',b
b1=b
ell_x = Sqrt(evals[0]) * 2.0
ell_y = Sqrt(evals[1]) * 2.0
if keyword_set(VERBOSE) then print,'semimajor and semiminor axis',ell_x,ell_y,' ratio= ',ell_x/ell_y
...
b = ((ATAN(evec[1], evec[0]) * 180. / !dpi - 90.0) +180 ) mod 180
; has to be > 0 (we do not want the symmetrical solution at -135°)
if keyword_set(VERBOSE) then print,'orientation (after correction)',b
b2=b
ell_x = Sqrt(evals[0]) * 2.0
ell_y = Sqrt(evals[1]) * 2.0
if keyword_set(VERBOSE) then print,'semimajor and semiminor axis',ell_x,ell_y,' ratio= ',ell_x/ell_y
