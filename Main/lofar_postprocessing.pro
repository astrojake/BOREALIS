...
;----------------------------------------------------------------------------------------------------------------
    ...
    for iij=0,n_elements(threshold_t)-1 do begin
     ...
     for iil=STARTELL,ENDELL do begin    ;for loop for Elliptical Correction 
      ...
      for iih=pol_start,pol_end do begin  
       If S eq 4 then begin ;Vprime
        ...
        If keyword_SET(LOFAR) then begin 
          ...
          UT_Rebin = UT_Rebin 
          If not(keyword_set(IMAGING)) then Ut_rebin  = UT_rebin(wt)
        endif  
      ...
      If keyword_SET(LOFAR) then begin 
        ...
        UT_Rebin2 = UT_Rebin 
        If not(keyword_set(IMAGING)) then Ut_rebin2  = UT_rebin(wt)
      endif  
      ...
      If keyword_set(Extra_Beam2) then begin
        ...
        xt_rebin1 = xt_rebin(wt)/3600.0d ;hours
         If keyword_SET(LOFAR) then begin 
          ...
          UT_Rebin4 = UT_Rebin 
          If not(keyword_set(IMAGING)) then Ut_rebin4  = UT_rebin(wt)
        endif  
  ...
  ;***********************************************************
     ...
     save,xf_rebin_q1,xf_q1,xfq1_diff,x2f_q1,x_q1,x2_q1,xd_q1,error_i_diff,error_i,error,error_diff,UT_rebin,filename=filename_ps+'FREQPLOT.sav'
