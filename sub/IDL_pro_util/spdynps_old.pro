...
;--------------------------------------------------------------------
  pro SPDYNPS_old, image,xmin,xmax,ymin,ymax,name_x,name_y,name_plot, $
       grid,backgr,thr,fracmin,fracmax,color,bar_titre, $
	log=log, zeromin=zeromin, iso=iso, screen=screen, newwin=newwin
;--------------------------------------------------------------------
...
; Dynamic range
  if (fracmin ne 0.) or (fracmax ne 1.) then begin
    ...
    if (tabmax eq -1) or (tabmin eq tabmax) then begin
  ...
  if keyword_set(screen) then begin
    ...
    if not keyword_set(log) then $
      plot, [xmin,xmax], [ymin,ymax], /nodata, xra=[xmin,xmax], xstyle=13, yra=[ymin,ymax], ystyle=13, title=name_plot, ticklen=tl, charsize=c, /device, pos=[i*xs1+63*f,j*ys1+45*f,i*xs1+63*f+n1,j*ys1+45*f+n2], iso=iso $
  ...
  if not keyword_set(screen) then begin
    ...
    plot, [0,1], [0,1], /nodata, xst=1, yst=12, pos=[(i+0.96)/nx-0.06,(j+0.1)/ny+0.075,(i+1.)/nx-0.06,(j+0.95)/ny-0.035], $
      /norm, xticklen=0, xticks=2, xtickv=[0,0.5,1], xtickname=bar_tick, xtitle=bar_titre, charsize=c
    axis, /yaxis, yra=[tabmin,tabmax], ticklen=-0.02, /ysty, charsize=0.9*c
    ...
    if not keyword_set(log) then $
      plot, [xmin,xmax], [ymin,ymax], /nodata, /noerase, xra=[xmin,xmax], xstyle=13, yra=[ymin,ymax], ystyle=13, title=name_plot, ticklen=tl, charsize=c, /norm, pos=[i*1./nx+0.06,j*1./ny+0.075,(i+0.92)/nx-0.06,(j+1.)/ny-0.035], iso=iso $
    else plot_io, [xmin,xmax], [ymin,ymax], /nodata, /noerase, xra=[xmin,xmax], xstyle=13, yra=[ymin,ymax], ystyle=13, title=name_plot, ticklen=tl, charsize=c, /norm, pos=[i*1./nx+0.06,j*1./ny+0.075,(i+0.92)/nx-0.06,(j+1.)/ny-0.035], iso=iso
    tv, tab, !x.window(0), !y.window(0), xsize=!x.window(1)-!x.window(0), ysize=!y.window(1)-!y.window(0),/normal
    axis, xaxis=0, xra=[xmin,xmax], xstyle=1, xticklen=tl, xtitle=name_x, charsize = c
    axis, xaxis=1, xra=[xmin,xmax], xstyle=1, xticklen=tl, xtickname=replicate(' ',9)
    axis, yaxis=0, yra=[ymin,ymax], ystyle=1, yticklen=tl, ytitle=name_y, charsize = c
