...
pro LE_AUTO, x,largeur,nsigma,pm, xnet,para, logfile, $
...
; Difference with LE_AUTO_S consists in addition of average values on both sides to correct edge effects. 
; This correction is adapted only for "flat" series of pixels.
...
width=largeur < n_elements(x)
  ...
  nx=n_elements(x)	& bord=fix(width/2) & tab=fltarr(bord)
  x1=x & testpred=0
  para=bytarr(nx+2*bord)+1b
  for i=1,maxiterations do begin
;   m=total(x1,/double)/nx
;   x1=[tab+m,x1,tab+m]
    x1=[tab+total(x1(0:bord-1))/bord,x1,tab+total(x1(nx-bord:nx-1))/bord]
    ...
    x1=xnet(bord:bord+nx-1)
    testpred=test
  endfor
fin:
  xnet=xnet(bord:bord+nx-1) & para=para(bord:bord+nx-1)
  ...
  if n_elements(logfile) ne 0 and ((i gt 10 and keyword_set(SAVSAMPLE)) or keyword_set(VERBOSE)) then begin
	...
	if keyword_set(SAVSAMPLE) ne 0 then $
	  printf,v,'Number of iterations = ',i,', ',long(total(1-para,/double)),' / ',long(n_elements(para)),'   ',strtrim(fix(SAVSAMPLE),2)+'.sav' $
  ...
  if keyword_set(SAVSAMPLE) ne 0 and (i gt 10 or keyword_set(VERBOSE)) then begin $
	save,x,largeur,width,nsigma,pm,xnet,para,i,MEDIAN,EDGE,NPIX,MAXITERATIONS,file='.'+sep+'samples'+sep+strtrim(fix(SAVSAMPLE),2)+'.sav'
